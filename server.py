...
async def agama_set_config(update: dict[str, Any]) -> dict[str, Any]:
    ...
    Example updates:
      Set locale:    {"l10n": {"language": "de_DE", "timezone": "Europe/Berlin"}}
      Set product:   {"software": {"product": "Leap"}}
      Set user:      {"users": {"firstUser": {"fullName": "Ada", "login": "ada", "password": "..."}}}

    Prefer this over agama_replace_config() unless you intend to wipe all settings.
    """
...
async def agama_run_action(action: Any) -> dict[str, Any]:
    ...
    Available actions (pass as string or object):
      "install"                                  — START INSTALLATION (destructive, irreversible)
      "probeStorage"                             — re-detect storage devices
      "activateStorage"                          — activate iSCSI/multipath/DASD
      "probeDASD"                                — re-probe DASD devices (s390x only)
      {"finish": "reboot"}                       — reboot after install
      {"finish": "halt"}                         — halt after install
      {"finish": "poweroff"}                     — power off after install
      {"finish": "stop"}                         — do nothing (stay in live env)
      {"configureL10n": {"language": "es_ES"}}   — change live system locale
      {"discoverISCSI": {"address": "...", ...}} — discover iSCSI nodes

    WARNING: "install" is DESTRUCTIVE and IRREVERSIBLE.
    Always call agama_issues() first and confirm with the user before triggering it.
    A 422 response means there are pending issues or the installer is busy.
    """
...
async def system_state() -> dict[str, Any]:
    ...
    Useful when the MCP server runs inside the live ISO itself — gives the AI
    ambient hardware context without an Agama API call. Also works on any
    openSUSE/SUSE machine to identify the running environment.

    Returns: OS name/version, architecture, CPU model + core count, RAM in GB,
    and block device listing.
    """
    ...
    mcp_log.info(f"[LLM-RES] system_state: os={state.get('os', {}).get('pretty_name','?')}")
    return state
